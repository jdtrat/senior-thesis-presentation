...
So this is great. We have a data frame, i.e. just a table, that can be saved and shared as a csv or excel spreadsheet, and we can create a survey just by plugging in the code.
...
.relative.tl.f3.ba.br3.shadow-3[
```{r, eval = FALSE}
...
To reiterate, here is a screenshot of the csv file, opened in Excel, that contains our two questions.

One of the benefits of our package is that you can store surveys as a csv or excel spreadsheet that can be read into R and create a survey. So, if one wanted to create a survey asking about your favorite food, we could do so with a csv like the table on the last slide. 
...
.tl.f3.relative.ba.br3.shadow-3.mt5.top-1[
```{r, eval = FALSE}
library(shiny)
library(shinysurveys)
...
Now, what does a survey like this look like? Well, https://bit.ly/3tkosOb
...
.mt6[
![Let's have a look!](https://jdtrat-apps.shinyapps.io/presentation-demo/)
]
